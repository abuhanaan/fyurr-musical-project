...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.rollback()
    flash('Unfortunately, Venue ' + name + 'could not be deleted')
...
def search_artists():
  ...
  results = Venue.query.filter(Venue.name.ilike('%{}%'.format
...
def show_artist(artist_id):
  ...
  try:
    ...
    data = {}
    # providing the data needed by the frontend
    data['id'] = artist_info.id
    data['name'] = artist_info.name
    data['genres'] = artist_info.genres
    data['city'] = artist_info.city
    data['state'] = artist_info.state
    data['phone'] = artist_info.phone
    data['website'] = artist_info.website
    data['facebook_link'] = artist_info.facebook_link
    data['seeking_venue'] = artist_info.seeking_description
    data['seeking_description'] = artist_info.seeking_description
    data['image_link'] = artist_info.image_link
    past_shows = []
    upcoming_shows = []
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M")

    past = db.session.query(Show).join(Venue).filter(db.and_(
      Show.start_time < current_time, Show.artist_id==artist_id)).all()
    for show in past:
      past_show = {}
      past_show['venue_id'] = show.venuee_id
      past_show['venue_name'] = show.venue.name
      past_show['venue_image_link'] = show.Venue.image_link
      past_show['start_time'] = str(show.start_time)
      past_shows.append(past_show)

    upcoming = db.session.query(Show).join(Venue).filter(db.and_(
                  Show.start_time > current_time, Show.artist_id == artist_id)).all()
    for show in upcoming:
      upcoming_show = {}
    ...
    flash(f"Unfortunately, the artist with the id {artist_id} is not in our database anymore",
          category="info")
    abort(404)  
...
def edit_artist_submission(artist_id):
  ...
  try:
    db.session.commit()
    flash(f"Artist {artist.name}'s record was updated successfully")
  except:
    db.session.rollback()
    flash(f"Unfortunately, artist {artist.name}'s record couldn't be updated successfully")
  finally:
    db.session.close()
...
def edit_venue_submission(venue_id):
  ...
  try:
    db.session.commit()
    flash(f"Venue {venue.name}'s record was successfully updated!", category="message")
  except:
    db.session.rollback()
    flash(f"Unfortunately, venue {venue.name}'s record couldn't be updated!", category="info")
  finally:
    db.session.close()
...
def create_show_submission():
  ...
  try:
    ...
    db.session.commit()
  # on successful db insert, flash success
    flash('Show was successfully listed!', category="message")
  # TODO: on unsuccessful db insert, flash an error instead.
  # e.g., flash('An error occurred. Show could not be listed.')
  # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/
  except:
    db.session.rollback
    flash('Show could not be listed, please try again later', category="info")
  return render_template('pages/home.html')
